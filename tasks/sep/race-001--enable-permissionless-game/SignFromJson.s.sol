...
contract SignFromJson is OriginalSignFromJson {
    ...
    GnosisSafe foundationSafe =
        GnosisSafe(payable(0xDEe57160aAfCF04c34C887B5962D0a69676d3C8B));
    ...
    function checkRespectedGameType() internal view {
        OptimismPortal2 portal = OptimismPortal2(
            payable(proxies.OptimismPortal)
        );
        ...
        returns (address[] memory)
    {
        ...
        address[] memory shouldHaveCodeExceptions = new address[](
            6 + securityCouncilSafeOwners.length
        );
        ...
        returns (Types.ContractSet memory _proxies)
    {
        ...
        l2OutputOracleProposer = stdToml.readAddress(
            chainConfig,
            "$.addresses.Proposer"
        );
        l2OutputOracleChallenger = stdToml.readAddress(
            chainConfig,
            "$.addresses.Challenger"
        );
        systemConfigOwner = stdToml.readAddress(
            chainConfig,
            "$.addresses.SystemConfigOwner"
        );
        batchSenderAddress = stdToml.readAddress(
            chainConfig,
            "$.addresses.BatchSubmitter"
        );
        p2pSequencerAddress = stdToml.readAddress(
            chainConfig,
            "$.addresses.UnsafeBlockSigner"
        );
        batchInboxAddress = stdToml.readAddress(
            chainConfig,
            "$.batch_inbox_addr"
        );
